...
- **Smart & Persistent History**: All your conversations are automatically saved. New chats are intelligently titled, and users can easily revisit, delete, or search through past discussions.
- **Searchable History**: Quickly find past conversations with a real-time search bar in the history sidebar.
- **Enhanced Error Handling**: API errors are displayed gracefully inline, with a "Try Again" button to resubmit your last prompt without losing context.
- **Customizable System Prompts**: Tailor the AI's personality and expertise for each chat session via a settings modal, giving you fine-grained control.
...
3.  **Contextual Prompting**: The application sends the new message along with the previous conversation history and any custom system prompt to the Gemini API.
